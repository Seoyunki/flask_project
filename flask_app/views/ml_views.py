...
@bp.route('/')
def index():
    return render_template('ml/ml_main.html')

@bp.route('/suggest/', methods=('GET', 'POST'))
def machine_learning():
    form = mlForm()

    userid = form.title.data
    ...
    if request.method == 'POST':
        ...
        mycontent = [movie for movie in recomm_movies.index]
...
@bp.route('/subsuggest/', methods=('GET', 'POST'))
def sub_learning():
    form = mlsubForm()
    ...
    if request.method == 'POST':
        ...
        ml = ml_movie_item()
        _, item_sim_df = ml.data_load()
        recomm_df = item_sim_df[movie_title].sort_values(ascending=False)[1:11]
        movie_list = [movie for movie in recomm_df.index]

        return render_template('ml/result_sub.html', movie_list=movie_list)

    return render_template('ml/suggest_sub.html', form=form)
